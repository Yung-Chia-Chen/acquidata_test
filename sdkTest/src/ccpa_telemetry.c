...
#define CONTROLLER_INFO02ACK 0x1E942048U
    ...
    switch (frame->id) {
        ...
        case CCPA_GENERAL_INFO00BRO:
            if (frame->dlc >= 6) {
                ...
                uint16_t pedalTorqueRaw = ccpa_u16_le(&frame->data[2]);
                if (pedalTorqueRaw != 0xFFFFU && pedalTorqueRaw != 0x0000U) {
                    state->riderTorqueNm = (float)pedalTorqueRaw * 0.01f;
                    state->riderTorqueValid = true;
                    state->riderTorqueSource = CCPA_TELEMETRY_SOURCE_GENERAL_INFO;
                    changed |= CCPA_TELEMETRY_CHANGED_RIDER_TORQUE;
                } else {
                    state->riderTorqueValid = false;
                    state->riderTorqueSource = CCPA_TELEMETRY_SOURCE_NONE;
                }
        ...
        case CCPA_CENTRAL_INTERFACE_ASSISTREQ:
            if (frame->dlc >= 2) {
        ...
        case CCPA_GENERAL_INFO01BRO:
            if (frame->dlc >= 5) {
                if (frame->data[4] != 0xFFU && frame->data[4] != 0x00U) {
        ...
        case CONTROLLER_INFO02BRO:
            if (frame->dlc >= 7) {
                state->motorRpm = ccpa_u16_le(&frame->data[4]);
                state->motorRpmValid = true;
                changed |= CCPA_TELEMETRY_CHANGED_MOTOR_RPM;

                state->motorTemperatureC = ccpa_temp_c(frame->data[6]);
                state->motorTemperatureValid = true;
                changed |= CCPA_TELEMETRY_CHANGED_MOTOR_TEMPERATURE;
            }
            break;
        ...
        case CONTROLLER_INFO03BRO:
            ...
            if (frame->dlc >= 6 && state->riderTorqueSource != CCPA_TELEMETRY_SOURCE_GENERAL_INFO) {
                state->riderTorqueNm = (float)ccpa_u16_le(&frame->data[4]) * 0.1f;
                state->riderTorqueValid = true;
                state->riderTorqueSource = CCPA_TELEMETRY_SOURCE_DEVICE;
                changed |= CCPA_TELEMETRY_CHANGED_RIDER_TORQUE;
            }
            break;
