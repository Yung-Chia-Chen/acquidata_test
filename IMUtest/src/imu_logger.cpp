...
void imu_logger_update()
{
  ...
  const float axG = static_cast<float>(raw.ax) / ACCEL_LSB_PER_G;
  const float ayG = static_cast<float>(raw.ay) / ACCEL_LSB_PER_G;
  const float azG = static_cast<float>(raw.az) / ACCEL_LSB_PER_G;
